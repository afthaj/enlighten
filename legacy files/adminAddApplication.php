  ...
  if (isset($_POST['submit'])) {
	...
	if (!empty($firstName) && !empty($lastName) && !empty($nationalID) && !empty($emailAddress) && !empty($ALIndexNumber) && !empty($ALYear) && !empty($OLIndexNumber) && !empty($OLYear) && !empty($adminDistrict) && !empty($preference01)) {
		...
		if (mysqli_num_rows($data) == 0) {
			// The user has not submitted an application previously. Therefore, an application can be submitted.
			...
			if ($row6 != NULL) {
				...
				$query2 = "INSERT INTO application (dateSubmitted, title, firstName, otherNames, lastName, nationalID, dateOfBirth, age, gender, contactAddress, telHome, telMobile, emailAddress, adminDistrict, nationality, race, religion, zScore, ALIndexNumber, ALYear, OLIndexNumber, OLYear, phyDisabilityFlag, numberOfAttempts) VALUES (NOW(), '$title', '$firstName', '$otherNames', '$lastName', '$nationalID', '$dateOfBirth', '$age', '$gender', '$contactAddress', '$telHome', '$telMobile', '$emailAddress', '$adminDistrict', '$nationality', '$race', '$religion', '" . $row6['zScore'] . "', '$ALIndexNumber', '$ALYear', '$OLIndexNumber', '$OLYear', '$phyDisabilityFlag', '" . $row6['numberOfAttempts'] . "')";
				...
				$msg = "Dear " . $_POST['firstName'] . " " . $_POST['lastName'] . ". Thank you for using the University Admission System. Your application has been successfully submitted. Your Index Number is " . $_POST['ALIndexNumber'] . ". Please check back with the UAS to view the outcome of the application.";
				
				mail($to, $subject, $msg);
		  
				// Confirm success with the user
				$error_msg = 'The application has been successfully submitted. You can <a href="adminViewAndEditApplication.php">edit</a> the application here. An email has been sent to the email address provided with important details.';
				...
				mysqli_close($dbc);
					...
					$error_msg = 'The AL Index Number provided does not match with the Department of Examination Database. Please apply with a valid AL Index Number and AL Year combination';
					}
		} else {
			  // The user has already submitted an application, therefore display an error message
			  $error_msg = 'This Applicant has already submitted an application. Please visit the <a href="adminViewAndEditApplication.php">Admin View/Edit Application page</a> to make any necessary changes.';
			  } 
	} else {
		$error_msg = 'All marked fields must be entered';
		}
	}
}
...
<head>
  <title>Add Application | UAS Admin</title>
  ...
  <script type="text/javascript">
      $(document).ready(function(){
          $("#adminAddApplicationForm").formToWizard({ submitButton: 'submit' })
      });
  </script>
    ...
    <h1>Add Application</h1>
      ...
      <div id="content">
			...
			if (isset($_SESSION['adminID'])) {
				...
				if ($_SESSION['privilegeLevel'] == "03" || $_SESSION['privilegeLevel'] == "02") {
			  ...
			  ?>
					  <form action="<? echo $_SERVER['PHP_SELF']; ?>" method="post" class="register" id="adminAddApplicationForm" name="adminAddApplicationForm">
						  <fieldset class="row2">
			  ...
			  <?php
					...
					echo '<p class="error_message">You do not have sufficient privileges to add a new Application</p>';
					}
                } else {
