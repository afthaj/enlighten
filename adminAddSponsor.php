  ...
  if (!isset($_SESSION['adminID'])) {
    if (isset($_COOKIE['adminID']) && isset($_COOKIE['username'])) {
	  ...
	  if (isset($_POST['submit'])) {
		...
		if (!empty($username) && !empty($password) && !empty($firstName) && !empty($lastName) && !empty($IDNumber) && !empty($emailAddress)) {
			
			$query = "INSERT INTO sponsor (username, password, joinDate, title, firstName, lastName, IDNumber, emailAddress) VALUES ('$username', SHA('$password'), NOW(), '$title', '$firstName', '$lastName', '$IDNumber', '$emailAddress')";
			
			mysqli_query($dbc, $query);
			
			  // Code to send the email
			  $to = $emailAddress;
			  $subject = 'You have been added as an Admin User for UAS';
			  $msg = "Dear $firstName $lastName.\n\nYou have been added as an Admin User in the University Admission System.\n\nYour Username is:  $username.\nYour Password is:  $password.\n\nPlease keep this information with you and use it to login to the UAS Admin Panel.";
			  
			  mail($to, $subject, $msg, 'From: Enlighten Admin');
	
			// Confirm success with the user
			$error_msg = 'The new Sponsor has been added.';
			  mysqli_close($dbc);
		  } else {
...
<head>
  <title>Add Sponsor | Enlighten Admin</title>
