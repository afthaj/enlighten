	  ...
	  if (isset($_POST['submit'])) {
		  ...
		  if (!empty($_POST['todelete'])) {
			  ...
			  foreach ($_POST['todelete'] as $delete_id) {
				  ...
				  $query = "DELETE FROM student WHERE studentID = '$delete_id'";
				  mysqli_query($dbc, $query) or die('Error querying database.');
				  
				  }
			
			  $error_msg = 'Student(s) removed.';
...
<head>
  <title>Remove Student | Enlighten Admin</title>
     ...
     <?php
			...
			if (isset($_SESSION['adminID'])) {
				...
				if ($_SESSION['privilegeLevel'] == "01") {
					...
					echo '<form action="' . $_SERVER['PHP_SELF'] . '" method="post" name="removeStudentForm" id="removeStudentForm">';
					echo '<br />';
					...
					while ($row = mysqli_fetch_array($result)) {
						...
						echo '<tr>';
						echo '<td>';
						echo '<input type="checkbox" value="' . $row['studentID'] . '" name="todelete[]" />';
						echo '</td>';
					...
					echo '<br />';
					echo '<label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>';
					echo '<input name="submit" id="submit" type="submit" value="Remove Student" />';
					echo '</p>';
					
					echo '</form>';

					} else {
						echo '<p class="error_message">You do not have sufficient privileges to remove a student</p>';
						}
				} else {
					// If the session var is empty, show any error message and the log-in form; otherwise confirm the log-in
