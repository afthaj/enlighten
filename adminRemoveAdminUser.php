...
<head>
  <title>Remove Admin User | Enlighten Admin</title>
     ...
     <?php
			...
			if (isset($_SESSION['adminID'])) {
				...
				if ($_SESSION['privilegeLevel'] == "01") {
					...
					echo '<form action="' . $_SERVER['PHP_SELF'] . '" method="post" name="removeAdminUserForm" id="removeAdminUserForm">';
					echo '<br />';
					...
					while ($row = mysqli_fetch_array($result)) {
						...
						echo '<tr>';
						echo '<td>';
						echo '<input type="checkbox" value="' . $row['adminID'] . '" name="todelete[]" />';
						echo '</td>';
					...
					echo '<br />';
					echo '<label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>';
					echo '<input name="submit" id="submit" type="submit" value="Remove User" />';
					echo '</p>';
