  ...
  if (!isset($_SESSION['adminID'])) {
    if (isset($_COOKIE['adminID']) && isset($_COOKIE['username'])) {
	  ...
	  if (isset($_POST['submit'])) {	  
		...
		if (!empty($firstName) && !empty($lastName) && !empty($dateOfBirth) && !empty($contactAddress)) {
			
			$query = "UPDATE student SET firstName = '$firstName', otherNames = '$otherNames', lastName = '$lastName', dateOfBirth = '$dateOfBirth', contactAddress = '$contactAddress', currentGrade = '$currentGrade', guardianName = '$guardianName', guardianTelephoneNumber = '$guardianTelephoneNumber', guardianContactAddress = '$guardianContactAddress' WHERE studentID = '$studentID'";
			
			mysqli_query($dbc, $query);
	
			// Confirm success with the user
			$error_msg = 'The Student has been updated.';
			mysqli_close($dbc);
		  } else {
...
<head>
  <title>View/Edit Student | Enlighten Admin</title>
    ...
    <div id="site_content">
	 ...
	 if (!empty($_GET['studentID'])) {
      ...
      <fieldset class="row3">
			...
			<br />
			<label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
			<input name="submit" id="submit" type="submit" value="Edit Student" />
		 ...
		 if ($_SESSION['privilegeLevel'] == "01") {
			 ...
			 if (empty($_GET['sponsorID'])) {
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					echo "<td><a href=\"adminViewAndEditStudent.php?studentID=" . $row['studentID'] . "\">Edit Student</a></td>";
					echo '</tr>';
					echo '</tbody>';
					}
				...
				echo '<p class="error_message">You do not have sufficient privileges to View/Edit a student</p>';
				}
